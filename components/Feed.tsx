...
const ActivityItem = memo(({ activity, user, isAdmin, onLike, onCommentClick, onDelete, activeCommentId, commentText, setCommentText, onSubmitComment, submittingComment, setDeleteTarget }: any) => {
    ...
    return (
        <div 
            className="bg-white dark:bg-stone-900 rounded-[2.5rem] shadow-sm border border-stone-100 dark:border-stone-800 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 animate-fade-in-up overflow-hidden"
        >
            <div className="p-8">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        <img src={activity.user.avatar} loading="lazy" className="w-12 h-12 rounded-full object-cover ring-2 ring-stone-100 dark:ring-stone-800" alt="" />
                        <div>
                            <p className="font-black text-stone-800 dark:text-stone-100 leading-none mb-1">{activity.user.name}</p>
                ...
                <div className="mt-8 pt-6 border-t border-stone-100 dark:border-stone-800 flex items-center gap-8">
                    <button 
                        ...
                        className={`flex items-center gap-2.5 transition-all group/btn ${isLiked ? 'text-rose-500 scale-110' : 'text-stone-400 hover:text-rose-500'}`}
                    >
                        <Heart size={20} className={isLiked ? 'fill-current' : 'group-hover/btn:scale-125 transition-transform'} />
                        <span className="text-xs font-black uppercase tracking-widest">{activity.likes}</span>
                    </button>
                    <button 
                        onClick={() => onCommentClick(showComments ? null : activity.id)}
                        className={`flex items-center gap-2.5 transition-all hover:text-stone-900 dark:hover:text-stone-100 ${showComments ? 'text-stone-900 dark:text-stone-100' : 'text-stone-400'}`}
                    >
                        <MessageSquare size={20} />
                        <span className="text-xs font-black uppercase tracking-widest">{activity.comments?.length || 0}</span>
                    </button>
                </div>
            </div>
            ...
            {showComments && (
                <div className="bg-stone-50 dark:bg-stone-950/50 p-8 border-t border-stone-100 dark:border-stone-800 animate-fade-in">
                    <div className="space-y-6 mb-8 max-h-[400px] overflow-y-auto custom-scrollbar pr-4">
                        {activity.comments?.map((comment: any) => (
                            <div key={comment.id} className="flex gap-4 group/comment animate-fade-in-up">
                                <img src={comment.userAvatar} loading="lazy" className="w-10 h-10 rounded-full object-cover shrink-0" alt="" />
                    ...
                    <div className="flex gap-4 items-center">
                        <img src={user.avatar} className="w-10 h-10 rounded-full object-cover shrink-0" alt="" />
                        <div className="flex-1 relative">
                            <input 
                                type="text"
                                value={commentText}
                                onChange={(e) => setCommentText(e.target.value)}
                                onKeyDown={(e) => e.key === 'Enter' && onSubmitComment(activity.id)}
                                placeholder="Оставить комментарий..."
                                className="w-full bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-2xl px-6 py-3 text-sm outline-none"
                            />
                            <button 
                                onClick={() => onSubmitComment(activity.id)}
                                disabled={!commentText.trim() || submittingComment === activity.id}
                                className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded-xl disabled:opacity-50"
                            >
                                {submittingComment === activity.id ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}
                            </button>
                        </div>
                    </div>
                </div>
...
export const Feed: React.FC<FeedProps> = ({ user, books, onRequireLogin, onPostCreated }) => {
      ...
      setActivities(prev => prev.map(act => {
          if (act.id === activityId) {
              ...
              return {
                  ...
                  likedBy: isLiked ? act.likedBy.filter(id => id !== user.id) : [...act.likedBy, user.id]
              };
          }
          return act;
      }));
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto pb-20">
      <ConfirmDialog 
        isOpen={!!deleteTarget}
        title={deleteTarget?.type === 'post' ? "Удалить пост" : "Удалить комментарий"}
      ...
      <div className="lg:col-span-2 space-y-8">
        ...
        <div className="bg-white dark:bg-stone-900 rounded-[2.5rem] p-8 shadow-sm border border-stone-100 dark:border-stone-800 animate-fade-in-up mb-10 relative overflow-hidden group">
            ...
            <div className={`flex flex-col gap-6 ${isGuest ? 'blur-sm' : ''}`}>
                <div className="flex gap-4">
                    <img src={user.avatar} alt={user.name} className="w-14 h-14 rounded-full object-cover ring-4 ring-stone-50 dark:ring-stone-800/50" />
                    <textarea
                        value={newPostContent}
                        onChange={(e) => setNewPostContent(e.target.value)}
                        placeholder="О чем вы думаете? Поделитесь мыслью..."
                        className="flex-1 bg-transparent border-none text-xl text-stone-800 dark:text-stone-100 placeholder:text-stone-300 dark:placeholder:text-stone-700 outline-none resize-none min-h-[100px] font-serif py-2"
                    />
                </div>
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-6 border-t border-stone-100 dark:border-stone-800">
                    <select 
                        value={selectedBookId}
                        onChange={(e) => setSelectedBookId(e.target.value)}
                        className="w-full md:w-64 bg-stone-50 dark:bg-stone-800 text-stone-600 dark:text-stone-300 text-[10px] font-black uppercase tracking-widest px-4 py-3 rounded-2xl outline-none border border-stone-200 dark:border-stone-700"
                    >
                    ...
                    <button
                        ...
                        className="w-full md:w-auto bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 px-10 py-3 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 transition-all disabled:opacity-50"
                    >
      ...
      <div className="hidden lg:block space-y-8">
        <div className="bg-white dark:bg-stone-900 p-8 rounded-[2.5rem] shadow-sm border border-stone-100 dark:border-stone-800 sticky top-24 animate-slide-in-right">
            <h3 className="font-serif font-black text-2xl text-stone-800 dark:text-stone-100 flex items-center gap-3 mb-8">
            ...
            <div className="space-y-6 mb-10">
                ...
                ) : leaderboard.map((reader, idx) => (
                    <div key={reader.id} className="flex items-center gap-4 group">
                        <div className={`font-black w-4 text-center text-sm ${idx === 0 ? 'text-amber-500' : 'text-stone-300 dark:text-stone-700'}`}>
                            {idx + 1}
                        </div>
                        <img src={reader.avatar} loading="lazy" alt={reader.name} className="w-12 h-12 rounded-full object-cover" />
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-black text-stone-800 dark:text-stone-100 truncate">{reader.name}</p>
                            <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-1">
                                <BookOpen size={10} /> {reader.booksReadThisYear} прочитано
                            </p>
                        </div>
                    </div>
                ))}
